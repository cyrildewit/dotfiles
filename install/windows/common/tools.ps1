...
.DESCRIPTION
    ...
    scoop has no formula and cask split, so a GUI application lands here too.
    bruno is the only one so far. Once there are several they earn their own
    applications.ps1, the way the macOS side keeps its casks apart.

    bruno is also the only entry outside the main bucket, and scoop knows about
    no other bucket until it is told. Adding extras clones it with git, which
